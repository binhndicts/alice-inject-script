...
export class MessageBridgeForRN extends MessageBridgeBase {
  ...
  public init() {
    this.setupCallbacks();
    // window.alert('Alice : setupCallbacks');
   }
  ...
  public injectWeb3Object() {
    ...
    // window.alert('Alice : web3 injected');
  }

  public injectEtheremObject() {
    window.ethereum = this._provider;

    // window.alert('Alice : window.ethereum injected');
  }

  private postMessage(data, callback) {
    data['timestamp'] = Date.now();
    // window.console('Alice : sendTransaction:' + JSON.stringify(data));
  ...
  private setupCallbacks() {
    window.alice = {
      callback: (response) => {
        ...
        // window.console('Alice : call back of sendTransaction:' + result);
